...
class PreTrainedTokenizerBase(PushToHubMixin):
    ...
    def add_special_tokens(
        ...
        if "additional_special_tokens" in special_tokens_dict:
            add_toks = special_tokens_dict.pop("additional_special_tokens")
            if "extra_special_tokens" in special_tokens_dict and special_tokens_dict["extra_special_tokens"]:
                existing = special_tokens_dict["extra_special_tokens"]
                if isinstance(existing, list) and isinstance(add_toks, (list, tuple)):
                    special_tokens_dict["extra_special_tokens"] = existing + list(add_toks)
                elif isinstance(existing, dict) and isinstance(add_toks, dict):
                    existing.update(add_toks)
            else:
                special_tokens_dict["extra_special_tokens"] = add_toks

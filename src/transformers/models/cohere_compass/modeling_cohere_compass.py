...
from .configuration_cohere_compass import CohereCompassConfig, CohereCompassTextConfig, CohereCompassVisionConfig
...
class CohereCompassLayerNorm(nn.Module):
...
@auto_docstring
class CohereCompassTextModel(CohereCompassPreTrainedModel):
...
@auto_docstring
class CohereCompassForCausalLM(CohereCompassPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >> from transformers import AutoTokenizer, CohereCompassForCausalLM

        >> model = CohereCompassForCausalLM.from_pretrained("CohereCompassForAI/c4ai-command-r-v01")
        >> tokenizer = AutoTokenizer.from_pretrained("CohereCompassForAI/c4ai-command-r-v01")
...
class CohereCompassVisionMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
...
class CohereCompassVisionPatchEmbed(nn.Module):
...
class CohereCompassVisionAttention(nn.Module):
    def __init__(self, config: CohereCompassVisionConfig) -> None:
        super().__init__()
...
class CohereCompassVisionBlock(GradientCheckpointingLayer):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        ...
        self.attn = CohereCompassVisionAttention(config=config)
        self.mlp = CohereCompassVisionMLP(config=config)
...
class CohereCompassVisionModel(CohereCompassPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.patch_embed = CohereCompassVisionPatchEmbed(
            config=config,
        )
        ...
        self.deepstack_merger_list = nn.ModuleList(
            [
                CohereCompassVisionPatchMerger(
                    config=config,
...
@auto_docstring
@dataclass
class CohereCompassModelOutputWithPast(BaseModelOutputWithPast):
    r"""
...
@auto_docstring
class CohereCompassModel(CohereCompassPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return CohereCompassModelOutputWithPast(
            **outputs,
...
class CohereCompassForConditionalGeneration(CohereCompassPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: CohereCompassConfig):
        super().__init__(config)
        self.model = CohereCompassModel(config)
